...
- `app/auth/page.tsx` - Email/password and Google OAuth sign-in/sign-up UI.
...
- `app/settings/page.tsx` - Account settings UI; profile/security use Supabase services, notification/simulation defaults are local UI state only.
...
- `app/settings/page.tsx` displays "Administrator" role and "Account created" as the current date; this does not appear to come from Supabase profile metadata.
- `app/auth/page.tsx` has a `name` field in registration mode, but `signUpWithEmail()` only sends email/password; the name is not passed to Supabase metadata.
...
- Update `README.md` and stale docs to reflect Mapbox/react-map-gl, current routes, and the hardcoded building-map decision.
...
- Decide whether registration should store the entered name in Supabase user metadata/profile.
